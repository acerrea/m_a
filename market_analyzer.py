...
def get_gemini_analysis(last_row, previous_row, df):
    ...
    try:
        ...
        # تشخیص وضعیت قدرت خریدار
        power_current = last_row['قدرت خريد']
        power_prev = previous_row['قدرت خريد']
        power_trend = "افزایش قدرت خریدار" if power_current > power_prev else "کاهش قدرت خریدار"

        prompt = f"""
        نقش: شما سردبیر کانال VIP دیتا بورس هستید(جمنای). مخاطبان شما سهامداران حرفه‌ای هستند که به دنبال "چرایی" اتفاقات بازارند، نه فقط اخبار خام.
        
        هدف: نوشتن یک گزارش تحلیلی جذاب، روان، و دارای ساختار بصری عالی (با استفاده هوشمندانه از ایموجی‌ها).

        📊 **داده‌های خام امروز ({last_row['تاریخ']}):**

        1️⃣ **جریان پول حقیقی (Smart Money):**
           - امروز: {money_current:+,.1f} میلیارد تومان
           - دیروز: {money_prev:+,.1f} میلیارد تومان
           - تغییر وضعیت: {money_change:+,.1f} میلیارد تومان نسبت به دیروز.
           
        2️⃣ **نبض معاملات (Volume):**
           - ارزش معاملات خرد: {vol_current:,.1f} همت
           - وضعیت نسبت به میانگین هفته: {vol_status_str} (این شاخص بسیار مهم است).
           
        3️⃣ **قدرت خریدار (Buyer Power):**
           - سرانه خریدار امروز: {power_current:.2f} (بالای 1.0 یعنی خوب، زیر 1.0 یعنی ضعیف).
           - روند نسبت به دیروز: {power_trend} (دیروز: {power_prev:.2f}).

        4️⃣ **تابلوی شاخص‌ها:**
           - شاخص کل: {last_row['شاخص کل']:,.0f} (تغییر: {(last_row['شاخص کل'] - previous_row['شاخص کل']):+,.0f} واحد)
           - شاخص هم‌وزن: {last_row['شاخص هم‌وزن']:,.0f} (تغییر: {(last_row['شاخص هم‌وزن'] - previous_row['شاخص هم‌وزن']):+,.0f} واحد)

        ✍️ **دستورالعمل نگارش (خیلی مهم):**
        
        1. **تیتر ژورنالیستی:** یک تیتر کوتاه و جذاب که خلاصه روز باشد. (مثلاً: "پاتک سنگین خریداران!" یا "تردید در سقف قیمتی؟")
        
        2. **پاراگراف اول (تصویر کلی):** با لحنی روان توضیح بده امروز در بازار چه گذشت. رابطه بین "ورود پول" و "شاخص" را تفسیر کن. اگر واگرایی وجود دارد (مثلاً شاخص مثبت ولی خروج پول داریم) حتماً اشاره کن.

        3. **بخش جزئیات (با ایموجی‌های دقیق):**
           - از ایموجی 💰 یا 💸 برای بخش پول استفاده کن. توضیح بده آیا پول هوشمند وارد شده یا خارج؟ شتاب آن کم شده یا زیاد؟
           - از ایموجی 📊 یا 📉 برای ارزش معاملات استفاده کن. آیا ارزش معاملات از روند صعودی حمایت می‌کند؟ (اگر ارزش معاملات کم باشد ولی بازار مثبت، این یک هشدار است).
           - از ایموجی 💪 یا 📉 برای قدرت خریدار استفاده کن. آیا زور خریداران بیشتر شده؟

        4. **سیگنال‌های مخفی (Market Depth):**
           - مقایسه کن: آیا سهم‌های کوچک (شاخص هم‌وزن) بهتر بودند یا شاخص‌سازها؟ این چه معنایی دارد؟

        5. **چشم‌انداز فردا (Actionable Advice):**
           - صریح بگو: فردا گارد صعودی داریم، نزولی یا نوسانی؟ استراتژی کاربر چه باشد؟ (خرید در منفی، سیو سود، یا تماشا؟)

        ⚠️ **نکات نگارشی:**
        - طول متن متعادل باشد (نه خیلی کوتاه، نه طومار).
        - جملات رباتیک نباشند. از اصطلاحات بازاری مثل "رنج مثبت"، "کد به کد"، "پول داغ"، "حمایت استاتیک" در صورت نیاز استفاده کن.
        - از بولت پوینت (•) استفاده کن تا متن خوانا باشد.
        """
