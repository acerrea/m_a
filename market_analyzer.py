...
def get_gemini_analysis(last_row, previous_row, df):
    ...
    try:
        ...
        # --- اصلاح بخش قدرت خریدار طبق منطق سایت منبع ---
        power_current = last_row['قدرت خريد']
        power_prev = previous_row['قدرت خريد']
        
        # تعیین وضعیت برای فهم بهتر هوش مصنوعی
        if power_current > 0:
            power_status = "🟢 قدرت دست خریداران است (مثبت)"
        elif power_current < 0:
            power_status = "🔴 قدرت دست فروشندگان است (منفی)"
        else:
            power_status = "⚪️ قدرت برابر (خنثی)"

        prompt = f"""
        نقش: شما سردبیر کانال VIP دیتا بورس هستید(جمنای).
        
        هدف: نوشتن یک گزارش تحلیلی جذاب و دقیق.
        ...
        3️⃣ **قدرت خریدار (نکته بسیار مهم):**
           - عدد شاخص قدرت: {power_current:.2f}
           - وضعیت فعلی: {power_status}
           - عدد دیروز: {power_prev:.2f}
           - **راهنمای تفسیر حیاتی:** در این داده‌ها، **عدد منفی** (مثل -1.26) یعنی **فروشندگان قوی‌ترند** و **عدد مثبت** یعنی خریداران قوی‌ترند. لطفاً این عدد را با معیار استاندارد (بالای 1) اشتباه نگیر. عدد منفی یعنی ضعف شدید تقاضا.

        4️⃣ **شاخص‌ها:**
           - شاخص کل: {last_row['شاخص کل']:,.0f} (تغییر: {(last_row['شاخص کل'] - previous_row['شاخص کل']):+,.0f})
           - شاخص هم‌وزن: {last_row['شاخص هم‌وزن']:,.0f} (تغییر: {(last_row['شاخص هم‌وزن'] - previous_row['شاخص هم‌وزن']):+,.0f})

        ✍️ **دستورالعمل نگارش:**
        
        1. **تیتر:** جذاب و متناسب با وضعیت (اگر قدرت منفی است، تیتر هشداری باشد).
        
        2. **تحلیل کلی:** اگر عدد قدرت خریدار منفی است ({power_current})، صراحتاً بگو که زور فروشندگان بیشتر بوده و خریداران عقب‌نشینی کرده‌اند.
        
        3. **جزئیات:**
           - ایموجی 💰 برای پول.
           - ایموجی 💪 برای قدرت خریدار: اگر منفی است، از واژگانی مثل "تضعیف جبهه خرید" یا "برتری فروشندگان" استفاده کن.

        4. **چشم‌انداز فردا:** بر اساس اینکه زور دست کیست (مثبت یا منفی)، پیش‌بینی کن.
        """
