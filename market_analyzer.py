...
# ... (تمام توابع دیگر مثل قبل اینجا قرار دارند، نیازی به تغییر آن‌ها نیست) ...
# ... (کپی کردن تمام توابع از get_gemini_analysis تا send_audio_to_telegram)
# --- تنظیمات اولیه ---
now = datetime.now()
...
def analyze_moving_averages(df):
    ...
    # Simple analysis based on position
...
def main():
    if not all([TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID]):
        print("❌ متغیرهای محیطی تلگرام (TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID) تنظیم نشده‌اند. برنامه متوقف می‌شود.")
        return
    ...
    try:
        ...
        # --- بررسی کلیدی: آیا جدول پیدا می‌شود؟ ---
        table = soup.find('table', class_='sticky market')
        if not table:
            print("❌❌❌ خطای بحرانی: جدول داده‌ها با کلاس 'sticky market' در صفحه یافت نشد. احتمالاً ساختار سایت تغییر کرده است.")
            # ارسال پیام خطا به تلگرام
            send_message_to_telegram(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, "🚨 **خطا در ربات تحلیل بازار** 🚨\n\nربات نتوانست جدول داده‌ها را از سایت منبع (TradersArena) استخراج کند. لطفاً ساختار سایت را بررسی کنید.\n\n@Data_Bors_Admin")
            return
    ...
    if len(data) < 2: 
        print("❌ داده کافی برای تحلیل مقایسه‌ای وجود ندارد (کمتر از ۲ روز داده دریافت شد).")
        return
    ...
    full_message_blocks = [] # ... (این بخش بدون تغییر است)
    # (بقیه کد ساخت پیام داده‌ها را اینجا کپی کنید)
    full_message_blocks = []
    ...
    for name, key in [('کل', 'شاخص کل'), ('هم‌وزن', 'شاخص هم‌وزن')]:
        ...
        dist_from_high = (current_idx - previous_yearly_high) / previous_yearly_high * 100 if previous_yearly_high else 0
    ...
    if ai_analysis_html:
        ...
        audio_filename = convert_text_to_speech(text_for_speech)
